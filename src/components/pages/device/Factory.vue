<template>
  <el-card class="box-card">
    <div>
      ...
      <!-- 角色数据表单 -->
      <el-table ref="multipleTable" :data="dataList" border style="width: 100%" height="500">
        <el-table-column label="供应商编号" prop="id" width="120">
        </el-table-column>
        <el-table-column label="供应商名称" prop="name" width="270">
        </el-table-column>
        <el-table-column label="联系电话" prop="phone" width="150">
        </el-table-column>
        <el-table-column label="设备型号" prop="type" width="250">
        </el-table-column>
        <el-table-column label="设备品牌" prop="brand" width="170">
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    //新增页面的确认键
    handleInsert() {
      this.$http.post("/device/company/insert", this.form).then(() => {
    ...
    getDataList() {
      ...
      this.$http.post("device/company/getList", pageInfo).then((res) => {
    ...
    handleDelete(item) {
      this.$confirm("是否确认删除?", "提示", {
        ...
        .then(() => {
          ...
          this.$http.post("/device/company/delete", data).then((res) => {
            this.cutDataList(res);
          });
        })
        .catch(() => {
    ...
    getDataListByName() {
      ...
      if (name !== "") {
        ...
        this.$http.post("/device/company/search", data).then((res) => {
          this.cutDataList(res);
        });
